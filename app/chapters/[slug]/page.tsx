    ...
    <main className="page-shell py-10 sm:py-14">
      ...
      <div className="mt-8 grid gap-12 lg:grid-cols-[14rem_minmax(0,46rem)] lg:justify-center lg:gap-16">
            ...
            {isIntroduction ? (
              <>
                {introductionSections.map((section) => (
                  <a key={section.id} href={`#${section.id}`} className="article-nav-link">
                    {section.title}
                  </a>
                ))}
                <a href="#self-check" className="article-nav-link">Перевір себе</a>
              </>
            ) : (
              <>
        ...
        <article className="min-w-0">
          <header id="overview" className="scroll-mt-28 border-b border-border pb-10">
            <div className="flex flex-wrap items-center gap-3">
              <span className="font-mono text-xs text-muted-foreground">
                {isIntroduction ? 'Вступ · ' : ''}Глава {String(chapter.order).padStart(2, '0')}
              </span>
          ...
          {nextChapter && (
            <footer className={cn('border-t border-border py-10', isIntroduction && 'border-t-0 pt-2')}>
              {isIntroduction ? (
                <Link
