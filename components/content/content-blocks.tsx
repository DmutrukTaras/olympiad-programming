    ...
    <div className="space-y-5">
      {blocks.map((block, index) => {
        if (block.type === 'paragraph') {
          return <p key={index} className="leading-7 text-muted-foreground">{block.text}</p>;
        }
        ...
        if (block.type === 'callout') {
          return (
            <aside key={index} className="rounded-2xl border border-primary/20 bg-primary/[0.06] p-5">
        ...
        return (
          <figure key={index} className="overflow-hidden rounded-2xl border border-border bg-[oklch(0.16_0.012_155)] text-[oklch(0.9_0.01_110)] shadow-sm">
            {block.caption && (
